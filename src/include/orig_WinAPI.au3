...
Func _WinAPI_GetOpenFileName($sTitle = "", $sFilter = "All files (*.*)", $sInitalDir = ".", $sDefaultFile = "", $sDefaultExt = "", $iFilterIndex = 1, $iFlags = 0, $iFlagsEx = 0, $hWndOwner = 0)
	...
	For $i = 1 To $asFLines[0]
		...
		$asFilter[$i * 2 - 1] = StringStripWS(StringLeft($asFLines[$i], $iStart - 1), 3);$STR_STRIPLEADING + $STR_STRIPTRAILING)
		$asFilter[$i * 2] = StringStripWS(StringTrimRight(StringTrimLeft($asFLines[$i], $iStart), StringLen($asFLines[$i]) - $iFinal + 1), 3);$STR_STRIPLEADING + $STR_STRIPTRAILING)
...
Func _WinAPI_PathFindOnPath(Const $sFilePath, $aExtraPaths = "", Const $sPathDelimiter = @LF)
	...
	If IsString($aExtraPaths) Then
		If StringLen($aExtraPaths) Then
			$aExtraPaths = StringSplit($aExtraPaths, $sPathDelimiter, 3);$STR_ENTIRESPLIT + $STR_NOCOUNT)
...
Func _WinAPI_ShowError($sText, $bExit = True)
	_WinAPI_MsgBox(4096, "Error", $sText)
	If $bExit Then Exit
...
Func _WinAPI_ShowMsg($sText)
	_WinAPI_MsgBox(4096, "Information", $sText)
